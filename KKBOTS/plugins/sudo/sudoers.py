...
from KKBOTS.utils.database import add_sudo, remove_sudo
from KKBOTS.utils.decorators.language import language
from KKBOTS.utils.extraction import extract_user
from KKBOTS.utils.inline import close_markup
from config import BANNED_USERS, OWNER_ID, PREFIXES



@app.on_message(filters.command(["addsudo"], prefixes=PREFIXES) & filters.user(OWNER_ID))
@language
...
@app.on_message(filters.command(["delsudo", "rmsudo"], prefixes=PREFIXES) & filters.user(OWNER_ID))
@language
...
photo_url = "https://telegra.ph/file/98554a5d03116b8a29b5f.jpg"

@app.on_message(filters.command(["sudolist", "listsudo", "sudoers"], prefixes=PREFIXES) & ~BANNED_USERS)
...
async def check_sudo_list(client, callback_query: CallbackQuery):
    ...
    if callback_query.from_user.id not in SUDOERS:
        return await callback_query.answer("» ᴏɴʟʏ ꜱᴜᴅᴏ ᴜꜱᴇʀꜱ ᴄᴀɴ ᴠɪᴇᴡ.", show_alert=True)
    else:
